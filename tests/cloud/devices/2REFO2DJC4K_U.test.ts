...
describe(MODEL_ID, () => {
    test('publishes the additive diagnostic components without removing existing controls', () => {
        ...
        assert.equal(components.selected_dispenser_type.platform, 'sensor')
        assert.equal(components.selected_dispenser_type.name, '현재 선택 출수 종류')
        assert.equal(components.selected_dispenser_type.command_topic, undefined)
    ...
    test('decodes confirmed and candidate read-only status fields', () => {
        ...
        assert.equal(properties.selected_dispenser_type, 'WATER')
