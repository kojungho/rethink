...
describe('H01', () => {
    test('publishes status, settings, operation, remote-start, energy, and diagnostic components', () => {
        ...
        assert.equal(components.rinse_level.entity_category, 'config')
        assert.equal(components.salt_level.entity_category, 'config')
        assert.equal(components.remote_mode.entity_category, 'config')
        assert.equal(components.clean_reminder.entity_category, 'config')
        assert.equal(components.end_alarm.entity_category, 'config')
        assert.equal(components.buzzer.entity_category, 'config')
    ...
    test('removes only retired or unsupported components before publishing the normal config', () => {
        ...
        assert.deepEqual(configs[0].rinse_level, { platform: 'number' })
        assert.deepEqual(configs[0].salt_level, { platform: 'number' })
        assert.deepEqual(configs[0].remote_mode, { platform: 'select' })
        assert.deepEqual(configs[0].clean_reminder, { platform: 'switch' })
        assert.deepEqual(configs[0].end_alarm, { platform: 'switch' })
        assert.deepEqual(configs[0].buzzer, { platform: 'select' })
