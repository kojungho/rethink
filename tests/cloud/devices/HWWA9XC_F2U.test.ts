...
describe('HWWA9XC_F2U', () => {
    ...
    test('decodes the additional captured cleaning and charging settings', () => {
        ...
        assert.equal(ha.devices[DEVICE_ID].properties.suction_power, 'SUCTION_LOW_POWER')

        thinq.emit('data', buf(MOP_AND_VACUUM))
        assert.equal(ha.devices[DEVICE_ID].properties.mop_mode, 'MOP_AND_VACUUM')

        thinq.emit('data', buf(WATER_LEVEL_2))
        assert.equal(ha.devices[DEVICE_ID].properties.water_supply, 'LEVEL_2')

        thinq.emit('data', buf(STEAM_SAFE))
        assert.equal(ha.devices[DEVICE_ID].properties.steam_mode, 'SAFE_STEAM')

        thinq.emit('data', buf(VERY_BRIGHT))
        assert.equal(ha.devices[DEVICE_ID].properties.display_brightness, 'BRIGHTNESS_VERY_BRIGHT')

        thinq.emit('data', buf(DUST_MELODY_BUTTERFLY))
        assert.equal(ha.devices[DEVICE_ID].properties.dust_empty_melody, 'BUTTERFLY')

        thinq.emit('data', buf(CHARGING_MELODY_NEBULA))
        assert.equal(ha.devices[DEVICE_ID].properties.charging_alert_volume, 'VOLUME_LOW')
        assert.equal(ha.devices[DEVICE_ID].properties.charging_melody, 'NEBULA')

        thinq.emit('data', buf(BUTTON_SOUND_MUTE))
        assert.equal(ha.devices[DEVICE_ID].properties.button_sound, 'VOLUME_MUTED')
    })
    ...
    test('sends every additional captured settings command', () => {
        ...
        dev.setProperty('suction_power', 'SUCTION_LOW_POWER')
        dev.setProperty('suction_power', 'SUCTION_NORMAL')
        dev.setProperty('suction_power', 'SUCTION_HIGH')
        dev.setProperty('suction_power', 'SUCTION_TURBO')
        dev.setProperty('mop_mode', 'MOP_AND_VACUUM')
        dev.setProperty('mop_mode', 'MOP_ONLY')
        dev.setProperty('water_supply', 'LEVEL_1')
        dev.setProperty('water_supply', 'LEVEL_2')
        dev.setProperty('steam_mode', 'SAFE_STEAM')
        dev.setProperty('steam_mode', 'HOT_WATER_MOP')
        dev.setProperty('display_brightness', 'BRIGHTNESS_DIM')
        dev.setProperty('display_brightness', 'BRIGHTNESS_NORMAL')
        dev.setProperty('display_brightness', 'BRIGHTNESS_BRIGHT')
        dev.setProperty('display_brightness', 'BRIGHTNESS_VERY_BRIGHT')
        dev.setProperty('dust_empty_melody', 'DEW')
        dev.setProperty('dust_empty_melody', 'SPROUT')
        dev.setProperty('dust_empty_melody', 'BUTTERFLY')
        dev.setProperty('charging_alert_volume', 'VOLUME_LOW')
        dev.setProperty('charging_alert_volume', 'VOLUME_NORMAL')
        dev.setProperty('charging_alert_volume', 'VOLUME_HIGH')
        dev.setProperty('charging_melody', 'LUCKY')
        dev.setProperty('charging_melody', 'MARBLE')
        dev.setProperty('charging_melody', 'ICE')
        dev.setProperty('charging_melody', 'BREEZE')
        dev.setProperty('charging_melody', 'NEBULA')
        dev.setProperty('button_sound', 'VOLUME_MUTED')
        dev.setProperty('button_sound', 'VOLUME_LOW')
        dev.setProperty('button_sound', 'VOLUME_NORMAL')
        dev.setProperty('button_sound', 'VOLUME_HIGH')
