...
describe('PAC_910604_WW', () => {
    ...
    test('decodes A8 sensors and A7 temperature', () => {
        ...
        assert.equal(ha.getProperty(DEVICE_ID, 'climate', 'fan_mode_state'), 'FAN_3')
        ...
        assert.equal(ha.devices[DEVICE_ID].properties.air_quality, 'AIR_QUALITY_GOOD')
    ...
    test('maps temperature step and detailed filter counters', () => {
        ...
        assert.equal(ha.getProperty(DEVICE_ID, 'temperature_step', 'state'), 'ONE_DEGREE')
    ...
    test('maps PAC-specific controls', () => {
        const { ha, thinq, dev } = makeDevice()
        ha.setProperty(DEVICE_ID, 'ai_dry', 'command', 'FAN_5')
        assert.match(thinq.outbox.at(-1)!.toString('hex'), /8390ff7c86/i)
        ha.setProperty(DEVICE_ID, 'climate', 'fan_mode_command', 'FAN_1')
        assert.match(thinq.outbox.at(-1)!.toString('hex'), /7e82/i)
        ha.setProperty(DEVICE_ID, 'climate', 'fan_mode_command', 'FAN_5')
        assert.match(thinq.outbox.at(-1)!.toString('hex'), /7e86/i)
        ha.setProperty(DEVICE_ID, 'ai_dry', 'command', 'AI_DRY_OFF')
        ...
        ha.setProperty(DEVICE_ID, 'climate', 'swing_mode_command', 'AIRFLOW_SPLIT')
        ...
        ha.setProperty(DEVICE_ID, 'air_quality_sensor', 'command', 'ALWAYS')
    ...
    test('maps captured standby-screen settings and dependent availability', () => {
        ...
        assert.equal(ha.getProperty(DEVICE_ID, 'standby_time_format', 'state'), 'HOUR_12')
        assert.equal(ha.getProperty(DEVICE_ID, 'standby_date', 'state'), 'ON')
        assert.equal(ha.getProperty(DEVICE_ID, 'standby_switch_interval', 'state'), 'MIN_5')
