...
describe(MODEL_ID, () => {
    test('config exposes expected HA components', () => {
        ...
        assert.equal(washerState.state_topic, '$this/washer/state')
        assert.equal(dryerState.state_topic, '$this/dryer/state')
        assert.equal(washerState.device_class, 'enum')
        assert.equal(dryerState.device_class, 'enum')
        assert.ok((washerState.options as string[]).includes('운전 중'))
        assert.ok((dryerState.options as string[]).includes('건조 중'))
